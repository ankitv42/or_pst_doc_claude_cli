...
CRITICAL: All numeric values must be computed numbers — never formulas or expressions.
Do NOT write "993 * 43.74" — compute and write "43424.82" instead.
...
Option A — Standard Replenishment (Even Spread):
    ...
    pool_id        = CP001 for most categories, CP004 for Electronics
    stores_served  = critical_count + at_risk_count (total affected stores)
    availability_pct:
        IF projected_demand > 0: MIN(100, (total_current_stock + order_qty) / projected_demand x 100)
...
Option B — Profit Maximisation (Tier-1 Stores Only):
    ...
    availability_pct:
        IF projected_demand > 0: MIN(100, (total_current_stock + order_qty) / projected_demand x 100)
        IF projected_demand = 0: use 60.0 (partial coverage — Tier-1 only)
    NOTE: If abc_class = 'A' → set not_recommended = true (full distribution required)

Option C — Expedite Air Freight:
    ONLY build if allows_expedite = true
    order_qty      = MAX(min_order_qty, projected_shortfall)
    lead_time_days = ROUND(effective_lead_time x 0.35, 3)
    total_cost_aed = order_qty x unit_cost_aed x (1 + expedite_premium_pct / 100)
    pool_id        = CP003 (always CP003 for expedite)
    stores_served  = critical_count + at_risk_count (all affected stores)
    availability_pct:
        IF projected_demand > 0: MIN(100, (total_current_stock + order_qty) / projected_demand x 100)
        IF projected_demand = 0: use 100.0 (full coverage, expedite priority)
...
════════════════════════════════════════════════════
Respond with ONLY valid JSON. No preamble. No explanation outside the JSON.
ALL NUMERIC VALUES must be final computed numbers — never formulas or expressions.
WRONG: "total_cost_aed": 993 * 43.74   RIGHT: "total_cost_aed": 43424.82
...
════════════════════════════════════════════════════
    ...
    - approval_amount_aed = winner option's total_cost_aed
                            ...
                            Example: if Option B total_cost_aed=25941.0, write 25941.0
    - approval_pool       = winner option's pool_id ("CP001", "CP003", "CP004")

    CRITICAL RULE: approval_amount_aed must ALWAYS equal the winning option's cost.
    It is NEVER 0. If you return 0 you have made an error — go back and fix it.
...
what event is approaching if any, why action is needed now]

THREE OPTIONS
Option A — [name]: AED [cost] from [pool]
[one line trade-off]
[ELIMINATED — reason: X] OR [FEASIBLE — score: X]

Option B — [name]: AED [cost] from [pool]
[one line trade-off]
[NOT RECOMMENDED — reason: X] OR [FEASIBLE — score: X]

Option C — [name]: AED [cost] from [pool]
[one line trade-off]
[FEASIBLE — score: X] OR [NOT RECOMMENDED — reason: X]

Mark the winner option (from capital_decision.recommended) with RECOMMENDED.
Mark eliminated options with ELIMINATED.
Mark not_recommended options with NOT RECOMMENDED.

ACTION REQUIRED
Approve Option [capital_decision.recommended] to authorise AED [capital_decision.approval_amount_aed] from [pool].
Supplier contact: [contact_name] — [contact_email] (from supplier_data)
